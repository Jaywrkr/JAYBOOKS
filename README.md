...
1. Copia `.env.example` a `.env.local` y añade `OPENAI_API_KEY`.
2. Ejecuta `npm install`.
3. Ejecuta `npm run dev`.

## Desplegar en Vercel

Importa el repositorio en Vercel y agrega `OPENAI_API_KEY` (y opcionalmente `OPENAI_MODEL`) en **Project Settings → Environment Variables**. Los libros se guardan actualmente en el almacenamiento local de cada navegador; conectar una base de datos como Supabase es el siguiente paso si se requiere una biblioteca compartida o multidispositivo.
